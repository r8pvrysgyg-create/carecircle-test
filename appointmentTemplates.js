...
export async function createAppointmentTemplate(templateData) {
  ...
  const docRef = await addDoc(collection(db, TEMPLATE_COLLECTION), {
    ...
    lastUsedAt: null,
    usedCount: Number(cleanTemplate.usedCount || 0),
...
export async function markTemplateUsed(templateId, currentUsedCount = null) {
  ...
  if (Number.isFinite(Number(currentUsedCount))) {
    updateData.usedCount = Number(currentUsedCount) + 1;
  }

  await updateDoc(doc(db, TEMPLATE_COLLECTION, templateId), updateData);
}
