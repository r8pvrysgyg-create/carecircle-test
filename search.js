...
function getMatches(term) {
  ...
  appState.directoryContacts.forEach(c => {
    const blob = `${c.name} ${c.category} ${c.organization} ${c.primaryContact} ${c.phone} ${c.extension} ${c.alternatePhone} ${c.fax} ${c.officeHours} ${c.email} ${c.website} ${c.street} ${c.city} ${c.state} ${c.zip} ${c.address} ${(c.linkedPeople || []).join(" ")} ${c.defaultAppointmentType} ${c.defaultLocation} ${c.typicalDuration} ${c.preferredDriver} ${c.transportationNotes} ${c.notes}`.toLowerCase();
    if (blob.includes(term)) {
      const phoneDetail = c.phone ? (c.extension ? `${c.phone} ext. ${c.extension}` : c.phone) : "";
      items.push({ type: "Directory", view: "directory", title: `${c.favorite ? "⭐ " : ""}${c.name || "Contact"}${c.category ? ` - ${c.category}` : ""}`, detail: phoneDetail || c.email || c.address || c.notes || "Directory contact" });
    }
  });
