...
function printDailySummary() {
  ...
  const html = `
    ...
    <html>
      <head>
        <title>CareCircle Care Packet</title>
        <meta charset="UTF-8" />
...
function buildDailySummaryHtml(forPrint) {
  ...
  if (options.mode === "weekly") {
    return buildWeeklySummaryHtml(options, forPrint);
  }

  return buildStandardSummaryHtml(options, forPrint);
}

function buildStandardSummaryHtml(options, forPrint) {
...
function buildWeeklySummaryHtml(options, forPrint) {
  const people = getSelectedPeople(options);
  const preparedTime = formatDateTimeSeattle(new Date());
  const days = getDateRangeKeys(options.startDate, options.endDate);

  const overviewHtml = renderWeeklyOverview(people, days, options);
  const daysHtml = days.map(day => renderWeeklyDay(day, people, options)).join("");

  return `
    <article class="daily-print weekly-print ${forPrint ? "print-mode" : ""}">
      <header class="print-header">
        <div>
          <p class="print-kicker">CareCircle</p>
          <h1>Weekly Care Packet</h1>
          <p class="print-date">${escapeHtml(getDateLabel(options))}</p>
        </div>
        <div class="prepared-box">
          <strong>Prepared</strong>
      ...
      <footer class="print-footer">
        CareCircle Weekly Care Packet • ${escapeHtml(getDateLabel(options))}
      </footer>
    </article>
  `;
}

function renderWeeklyOverview(people, days, options) {
  if (!people.length) return `<p>No people selected.</p>`;

  const rows = people.map(person => {
    const appointments = getAppointmentsForPersonAndRange(person, options.startDate, options.endDate);
    const transportationCount = appointments.reduce((count, appointment) => {
      return count + (appointment.maplePickupStart || appointment.maplePickupEnd ? 1 : 0) +
        (appointment.returnPickupStart || appointment.returnPickupEnd ? 1 : 0);
    }, 0);
    ...
    return `
      <div class="weekly-overview-card">
        <h3>${person === "Grandma" ? "👵" : "👴"} ${escapeHtml(person)}</h3>
        <p><strong>${appointments.length}</strong> appointments</p>
        <p><strong>${transportationCount}</strong> transportation windows</p>
        <p><strong>${tasks.length}</strong> open tasks</p>
        <p><strong>${medications.length}</strong> active medications</p>
      </div>
    `;
  }).join("");

  return `
    <section class="weekly-overview print-block">
      <h2>Weekly Overview</h2>
      <p class="muted-print">${escapeHtml(days.length)} day care packet from ${escapeHtml(formatDateLong(options.startDate))} through ${escapeHtml(formatDateLong(options.endDate))}.</p>
      <div class="weekly-overview-grid">${rows}</div>
    </section>
  `;
}

function renderWeeklyDay(day, people, options) {
  const peopleSections = people.map(person => renderWeeklyPersonDay(person, day, options)).join("");

  return `
    <section class="weekly-day-section">
      <h2>${escapeHtml(formatDateLong(day))}</h2>
      ${peopleSections || `<p class="empty-line">No people selected.</p>`}
    </section>
  `;
}

function renderWeeklyPersonDay(person, day, options) {
  const dayOptions = { ...options, startDate: day, endDate: day, date: day };
  const appointments = getAppointmentsForPersonAndRange(person, day, day);
  const tasks = getTasksForPersonAndRange(person, day, day);
  const medications = getMedicationsForPerson(person);
  const notes = getNotesForPerson(person);

  const transportation = renderTransportationSection(appointments);
  const appointmentsHtml = renderAppointmentsSection(appointments);
  const medicationsHtml = renderMedicationChecklist(medications);
  const tasksHtml = renderTasksSection(tasks);
  const notesHtml = renderNotesSection(notes);

  const hasVisibleItems = appointments.length || tasks.length || medications.length || (options.includeNotes && notes.length);

  return `
    <div class="weekly-person-day">
      <h3>${person === "Grandma" ? "👵" : "👴"} ${escapeHtml(person)}</h3>
      ${!hasVisibleItems ? `<p class="empty-line">No scheduled care items for this day.</p>` : ""}
      ${options.includeTransportation ? transportation : ""}
      ${options.includeAppointments ? appointmentsHtml : ""}
      ${options.includeMedications ? medicationsHtml : ""}
      ${options.includeTasks ? tasksHtml : ""}
      ${options.includeNotes ? notesHtml : ""}
    </div>
  `;
}
...
function getPrintOptions() {
  const checkedMode = document.querySelector('input[name="printMode"]:checked')?.value;
  const mode = checkedMode === "weekly" || checkedMode === "range" ? checkedMode : "single";
  ...
  if (mode === "range") {
    startDate = minDate(rawStart, rawEnd);
    endDate = maxDate(rawStart, rawEnd);
  }

  if (mode === "weekly") {
    startDate = singleDate;
    endDate = addDaysToDateKey(singleDate, 6);
  }
...
function getDateLabel(options) {
  if (options.mode === "weekly") {
    return `Week of ${formatDateLong(options.startDate)} through ${formatDateLong(options.endDate)}`;
  }
...
function addDaysToDateKey(value, daysToAdd) {
  const parts = getDateParts(value);
  if (!parts) return value;

  // Use local noon to avoid timezone/DST edge cases while still treating
  // input values as calendar dates, not UTC timestamps.
  const date = new Date(parts.yearNumber, parts.monthNumber - 1, parts.dayNumber + daysToAdd, 12, 0, 0);
...
function getPrintStyles() {
  return `
    ...
    .weekly-overview { page-break-inside: avoid; margin-bottom: 22px; }
    .weekly-overview h2 { margin: 0 0 8px; font-size: 22px; }
    .muted-print { color: #475569; margin: 0 0 12px; }
    .weekly-overview-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
    .weekly-overview-card { border: 1px solid #cbd5e1; border-radius: 10px; padding: 12px; }
    .weekly-overview-card h3 { margin: 0 0 8px; }
    .weekly-overview-card p { margin: 4px 0; }
    .weekly-day-section { page-break-before: always; border-top: 3px solid #111827; padding-top: 14px; margin-top: 22px; }
    .weekly-day-section:first-of-type { page-break-before: auto; }
    .weekly-day-section > h2 { margin: 0 0 14px; font-size: 24px; }
    .weekly-person-day { page-break-inside: avoid; border: 1px solid #cbd5e1; border-radius: 10px; padding: 12px; margin: 12px 0; }
    .weekly-person-day > h3 { margin: 0 0 10px; font-size: 19px; }
