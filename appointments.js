...
export function setupAppointmentForm() {
  ...
  if (appointmentTime) {
    appointmentTime.onchange = () => {
      const selectedTemplate = appointmentTemplates.find(t => t.id === templateSelect?.value);
      if (selectedTemplate) {
        applyTemplateTiming(selectedTemplate);
      }
    };
  }
...
async function applySelectedAppointmentTemplate(templateId) {
  ...
  applyTemplateTiming(template);

  await markTemplateUsed(templateId, template.usedCount || 0);
...
function applyTemplateTiming(template) {
  const appointmentTime = document.getElementById("apptTime");
  ...
  if (endTime && (!endTime.value || endTime.dataset.autoDefault === "true")) {
  ...
  if (mapleStart && (!mapleStart.value || mapleStart.dataset.autoDefault === "true")) {
  ...
  if (mapleStart?.value && mapleEnd && (!mapleEnd.value || mapleEnd.dataset.autoDefault === "true")) {
    mapleEnd.value = addMinutesToTime(mapleStart.value, 30);
    mapleEnd.dataset.autoDefault = "true";
  }

  if (returnStart && (!returnStart.value || returnStart.dataset.autoDefault === "true")) {
  ...
  if (returnStart?.value && returnEnd && (!returnEnd.value || returnEnd.dataset.autoDefault === "true")) {
