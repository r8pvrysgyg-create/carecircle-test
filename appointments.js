...
export function setupAppointmentForm() {
  ...
  if (saveAsTemplate && templateNameRow) {
    saveAsTemplate.onchange = () => {
      templateNameRow.style.display = saveAsTemplate.checked ? "block" : "none";
    };
  }
...
async function applySelectedAppointmentTemplate(templateId) {
  ...
  setValue("apptDoctor", template.doctor || "");
  setValue("apptLocation", template.location || "");
  setValue("apptNotes", template.notes || "");

  setDriverFromTemplate(template.preferredDriver);
